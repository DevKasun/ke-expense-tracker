...
export function MonthlyComparisonChart({ months = 6 }: MonthlyComparisonChartProps) {
    ...
    const highestMonth = data.reduce((max, item) => item.amount > max.amount ? item : max, data[0] || { amount: 0, month: '' })
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-4 text-center">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-semibold">{formatCurrency(highestMonth.amount)}</p>
                        </div>
                    </div>
                                        ...
                                        <ChartTooltipContent 
                                            ...
                                            labelFormatter={(label: string, payload: any) => {
                                                if (payload && payload.length > 0) {
                                                    ...
                                                    return `${label} - ${item.count} transaction${item.count !== 1 ? 's' : ''}`
                                                }
                                                return label
                                            }}
                                        />
